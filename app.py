...
@app.route('/check_consent')
def check_consent():
    return user_data['consent']
...
def send_consent_email(receiver_email, username):
    BASE_URL = 'https://subrotund-unpurposely-lula.ngrok-free.dev'

    allow_url = f"{BASE_URL}/consent/allow"
    deny_url = f"{BASE_URL}/consent/deny"
...
def consent_allow():
    ...
    return "Consent Approved. You may close this tab."


@app.route('/consent/deny')
def consent_deny():
    user_data['consent'] = 'denied'
    return "Consent Denied. You may close this tab."

@app.route('/consent_result/approved')
def consent_result_approved():
    return render_template(
        'final.html',
        result="Consent Approved ✅",
        message="Deepfake generation allowed."
    )


@app.route('/consent_result/denied')
def consent_result_denied():
    return render_template(
        'final.html',
        result="Consent Denied",
        message="Deepfake attempt blocked."
    )
